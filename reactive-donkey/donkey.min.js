...
function(a,b){var c=b[0],d=b[1];a=velocity(a);intersects(a,d)&&!a.gameOver&&(a.gameOver=!0,a.id="pinkie gameover",a.vy=-20,(new Audio("../../media/sound/gameover.mp3")).play(),--a.lives);if(a.gameOver)return a.vy+=.5,a;a.vy+=.88;0<=a.y&&0<a.vy&&(a.id="pinkie",a.y=0,a.vy=0);"space"===c[0]||"up"===c[0]||"touchstart"===c[0]?0===a.y&&(a.id="pinkie jumping",a.vy=-22,(new Audio("../../media/sound/jump.mp3")).play()):void 0!=c&&void 0!=c[0]&&alert("keys: "+c[0]);return a}).takeWhile(onscreen),initialCoin=
{id:"coin",vx:-6,vy:0,x:1600,y:40,points:0},coinStream=pinkieStream.scan(initialCoin,function(a,b){a=velocity(a);0>a.vy&&(a.vy*=2);0===a.vy&&intersects(a,b)&&((new Audio("../../media/sound/coin.mp3")).play(),a.vx=0,a.vy=-1,a.points+=1);return-300>a.x?initialCoin:-1E3>a.y?(v=velocity(initialCoin),v.vx*=a.points+1,v.points=a.points,v):a}),initialStat={id:"stat",x:0,y:0,points:0,text:"Init.."},statStream=coinStream.scan(initialStat,function(a,b){-1===b.vy&&(a.points+=b.points);a.text="Points: "+a.points;
return a});(new Audio("../../media/music/coldwet.mp3")).play();Rx.Observable.zipArray(groundStream,pinkieStream,coinStream,statStream,haterStream).subscribe(renderScene);
