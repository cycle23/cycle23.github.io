...
function onscreen(a){return!(-300>a.x||-1E3>a.y||1E3<a.y)}function updateElement(a){$(a.id).attr("class",a.classes||"").css({left:coordBI(a.x,a.baseX)+"px",top:coordBI(a.y,a.baseY)+"px"}).text(a.text)}function velocity(a){return assoc(a,{x:a.x+a.vx,y:a.y+a.vy})}function intersects(a,b){var c=coordBI(b.x,b.baseX),d=coordBI(a.x,a.baseX),f=coordBI(b.y,b.baseY),e=coordBI(a.y,a.baseY);return 100>Math.abs(d-c)&&100>Math.abs(e-f)}
function makeElement(a){return React.DOM.div({className:a.id,style:{left:a.x+(a.baseX||0)|NaN,top:a.y+(a.baseY||0)|NaN}})}function renderScene(a){return React.renderComponent(React.DOM.div(null,a.map(makeElement)),canvas)}function bindKey(a){var b=new Rx.Subject;Mousetrap.bind(a,function(){b.onNext(a)});return b}
...
!0,a.id="pinkie gameover",a.points=0,a.vy=-20,(new Audio("../../media/sound/gameover.mp3")).play());if(a.gameOver)return a.vy+=.5,a.points+=1,a;a.vy+=.98;0<=a.y&&0<a.vy&&(a.id="pinkie",a.y=0,a.vy=0);"space"===c[0]&&0===a.y&&(a.id="pinkie jumping",a.vy=-20,(new Audio("../../media/sound/jump.mp3")).play());return a}).takeWhile(onscreen),initialCoin={id:"coin",vx:-6,vy:0,x:1600,y:40},coinStream=pinkieStream.scan(initialCoin,function(a,b){a=velocity(a);0===a.vy&&intersects(a,b)&&((new Audio("../../media/sound/coin.mp3")).play(),
a.vx=0,a.vy=-1);0>a.vy&&(a.vy*=2);return onscreen(a)?a:initialCoin}),initialStat={id:"stat",x:0,y:0,points:0,text:"Init.."},statStream=pinkieStream.scan(initialStat,function(a,b){a.text="Points: "+b.points;return a});Rx.Observable.zipArray(groundStream,pinkieStream,coinStream,statStream,haterStream).subscribe(renderScene);
